...
extern volatile Uint16 SCIATxBuff[DSP_SCIA_TX_MAX_BUFFER_LENGTH];
extern volatile Uint16 SCIARxBuff[DSP_SCIA_RX_MAX_BUFFER_LENGTH];
...
{
	GpioDataRegs.GPASET.bit.GPIO30 = 1; //cpu 1 설정.
	asm(" RPT #40 || NOP");
}
...
E_DSP_SCIA_FLAG DSP_SCIA_RxUpDate(volatile Uint16 *Data, volatile Uint16 *Buff, volatile Uint16 *Count, volatile Uint16 *Channel)
{
		...
		Uint16 channel_temp=0;
		Uint16 channel_start_index_temp=0;
		Uint16 channel_end_index_temp=0;
			...
			(Buff[5] == (DSP_SCIA_RUN_SYNC_FLAG & 0x00FF) || Buff[5] == (DSP_SCIA_DATA_CALL_FLAG & 0x00FF)))
		{
			...
			if(CRC16 != Data[DSP_SCIA_RUN_SYNC_LENGTH - DSP_SCIA_CRC16_LENGTH]){
			    return(SCIA_ERROR_CRC16);
			}
			channel_temp = (*Channel) = (Data[DSP_SCIA_START_FLAG_LENGTH] & 0x000F);
			channel_start_index_temp = CONTROL_CHANNEL_START_INDEX;
			channel_end_index_temp = CONTROL_CHANNEL_END_INDEX;
            if (((*Channel) > CONTROL_CHANNEL_END_INDEX ) && ((*Channel) != 0x000F)) {
                return (SCIA_ERROR_CHANNEL);
            }
		...
		if(!((*Count) >= DSP_SCIA_RX_MAX_BUFFER_LENGTH)){
		    return(SCIA_ERROR_DATA);
		}
		...
		if(CRC16 != Data[DSP_SCIA_RX_MAX_DATA_LENGTH - DSP_SCIA_CRC16_LENGTH]){
		    return(SCIA_ERROR_CRC16);
		}
		(*Channel) = (Data[DSP_SCIA_START_FLAG_LENGTH] & 0x00FF);
        if ((*Channel) < CONTROL_CHANNEL_START_INDEX || (*Channel) > CONTROL_CHANNEL_END_INDEX) {
            return (SCIA_ERROR_CHANNEL);
        }
